...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Simple validation
    if (empty($name) || empty($amount) || empty($period) || empty($coverage) || empty($longitude) || empty($latitude) || empty($login_id)) {
    ...
    try {
        ...
        $stmt2 = $conn->prepare("INSERT INTO scheme (name, amount, period, coverage, login_id) VALUES (?, ?, ?, ?, ?)");
        $stmt2->bind_param("sssss", $name, $amount, $period, $coverage, $login_id);
